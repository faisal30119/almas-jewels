    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-[480px] bg-white rounded-md shadow-2xl overflow-hidden z-10 p-6 sm:p-8 md:p-10 my-auto text-neutral-900 border border-neutral-100"
        >
          {/* Close Button */}
          <button
            onClick={handleClose}
          ...
          <div className="mb-6">
            ...
            {mode !== 'register' && (
              <p className="text-sm text-neutral-500 mt-1 font-sans">
                ...
                {mode === 'forgot' && "We'll send you an email to reset your password."}
              </p>
            )}
          </div>
          ...
          {/* Feedback Banners */}
          {error && (
            <div className="mb-5 p-3 rounded bg-red-50 border border-red-200 text-red-700 text-xs sm:text-sm font-sans leading-relaxed">
              {error}
            </div>
          )}

          {successMsg && (
            <div className="mb-5 p-3 rounded bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs sm:text-sm font-sans leading-relaxed">
              {successMsg}
            </div>
          )}
          ...
          <div className="mt-6 pt-2 space-y-3 font-sans">
            ...
            {mode === 'forgot' && (
              <button
                onClick={() => switchMode('login')}
                className="group w-full text-left font-medium text-neutral-900 hover:text-[#D4A359] transition-colors flex items-center gap-1 py-1 text-sm sm:text-base border-b border-neutral-900 pb-1 w-max"
              >
